...
const MODE_SIGNALS: Record<InteractionMode, ModeSignals> = {
  ...
  'MODE-CRD': {
    keywords: [
      ...
      'write me', 'send them', 'tell them', 'let them know',
      'write an email', 'send an email', 'respond to this email',
      'respond to this', 'reply to this', 'client message',
      'write a response', 'craft', 'put together',
      'memo', 'notice response', 'irs response',
    ],
    patterns: [
      /draft (a|an|the|this) (email|letter|response|reply|message)/i,
      /write (a|an|the|this)?\s*(email|letter|response|reply|message)/i,
      /write me (a|an|the)?\s*(email|letter|response|reply|message)/i,
      /send (a|an|the|this)?\s*(email|letter|message|response)/i,
      /how (should|do|can) (i|we) (respond|reply|word|phrase)/i,
      /compose (a|an|the)/i,
      /help me (write|draft|respond|reply|craft)/i,
      /what (should|do) (i|we) (say|tell|write)/i,
      /respond to (this|the|their|that|an?) (email|message|letter|notice|inquiry)/i,
      /reply to (this|the|their|that|an?) (email|message|letter|notice)/i,
      /tell (the |this |my )?(client|them|him|her) (that|about|we|i)/i,
      /let (the |this |my )?(client|them|him|her) know/i,
      /write .*\b(to|for)\b.*(client|customer)/i,
      /put together (a|an|the)?\s*(email|letter|message|response)/i,
      /craft (a|an|the)?\s*(email|letter|message|response)/i,
      /email .* (about|regarding|re:|for)/i,
    ],
